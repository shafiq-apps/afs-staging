    ...
    if (filters && !$.empty(filters)) {
      Object.keys(filters).forEach((key) => {
        ...
        if (Array.isArray(value) && value.length > 0) {
          ...
          const min = typeof priceRange.min === "number" && !isNaN(priceRange.min) ? Math.round(priceRange.min) : void 0;
          const max = typeof priceRange.max === "number" && !isNaN(priceRange.max) ? Math.round(priceRange.max) : void 0;
...
var API = {
  ...
  shouldSendCpid() {
    var _a;
    return !!((_a = FilterState.selectedCollection) == null ? void 0 : _a.id);
  },
  async products(filters, pagination, sort) {
    ...
    if (this.shouldSendCpid()) {
      ...
      Log.debug("cpid not sent to products API", {
        hasCpid: !!((_a = FilterState.selectedCollection) == null ? void 0 : _a.id),
        filters,
...
var DOM = {
    ...
    DOM.container.addEventListener("click", (e) => {
      ...
      if (action === "clear-all") {
        ...
        if (FilterState.filters[handle]) {
          if (isCollectionFilter && ((_f = FilterState.selectedCollection) == null ? void 0 : _f.id)) {
  ...
  createPriceRangeGroup(filter, savedStates = null) {
    ...
    header.appendChild(toggle);
    const isPriceRangeActive = FilterState.filters.priceRange && (FilterState.filters.priceRange.min !== minRange || FilterState.filters.priceRange.max !== maxRange);
    ...
    Object.keys(filters).forEach((key) => {
      ...
      if ($.equals(key, "search" /* SEARCH */) && value && typeof value === "string" && value.trim()) {
        ...
        if (hasMin && hasMax) {
          ...
          activeFilters.push({ handle: key, label: `${Lang.labels.price}$${roundedMin} - $${roundedMax}`, value: "__clear__" /* CLEAR */ });
        } else if (hasMin) {
          const roundedMin = Math.round(priceRange.min);
          activeFilters.push({ handle: key, label: `${Lang.labels.price}$${roundedMin}+`, value: "__clear__" /* CLEAR */ });
        } else if (hasMax) {
          const roundedMax = Math.round(priceRange.max);
          activeFilters.push({ handle: key, label: `${Lang.labels.price}Up to $${roundedMax}`, value: "__clear__" /* CLEAR */ });
        }
...
var Filters = {
  ...
  toggle(handle, value) {
    ...
    if (isCollectionFilter && ((_a = FilterState.selectedCollection) == null ? void 0 : _a.id)) {
      const originalCpid = FilterState.selectedCollection.id;
