...
var SearchState = {
  ...
  observedInputs: /* @__PURE__ */ new Set()
};
var SearchDOM = {
...
var SearchInit = {
  ...
  overrideSearchButton(button) {
    ...
    clonedButton.addEventListener("click", (e) => {
      ...
      const searchInput = document.querySelector('input[name="q"], input[name="search"], input[type="search"]');
      if (searchInput) {
        searchInput.focus();
      }
    }, true);
    SearchState.searchButtons.add(clonedButton);
    Log.debug("Replaced search button with clone to remove native event listeners (kept visible)", { button: clonedButton });
  },
  /**
  ...
  overrideSearchContainers() {
    ...
    containers.forEach((container) => {
      ...
      Array.from(container.attributes).forEach((attr) => {
        if (attr.name !== "data-afs-hidden") {
          clonedContainer.setAttribute(attr.name, attr.value);
        }
      });
      if (container.parentNode) {
        container.parentNode.replaceChild(clonedContainer, container);
      }
      clonedContainer.setAttribute("data-afs-hidden", "true");
      SearchState.overriddenContainers.add(clonedContainer);
      Log.debug("Removed event listeners from search container (kept visible)", { container: clonedContainer });
    });
  },
  replaceInput(input) {
    ...
    if (input.parentNode) {
      ...
      newInput = clonedInput;
      Log.debug("Replaced search input with clone to remove native event listeners", {
        inputName: newInput.name,
        hasParent: !!newInput.parentNode
      });
    } else {
      Log.warn("Input has no parent node, using original input", { input });
      newInput = input;
    }
    ...
    if (form && !form.hasAttribute("data-afs-form-replaced")) {
      const clonedForm = form.cloneNode(true);
      clonedForm.action = form.action;
      clonedForm.method = form.method;
      clonedForm.enctype = form.enctype;
      clonedForm.setAttribute("data-afs-form-replaced", "true");
      const clonedInputInForm = clonedForm.querySelector(`input[name="${newInput.name}"], input[type="search"]`);
      if (clonedInputInForm) {
        clonedInputInForm.value = newInput.value;
        clonedForm.replaceChild(newInput, clonedInputInForm);
      }
      if (form.parentNode) {
    ...
    newInput.addEventListener("input", SearchLogic.handleInput);
    newInput.addEventListener("focus", SearchLogic.handleFocus);
    newInput.addEventListener("blur", SearchLogic.handleBlur);
    newInput.addEventListener("keydown", SearchLogic.handleKeyDown);
   ...
   */
  observeInputForVisibility(input) {
    ...
    if (!SearchState.visibilityObserver) {
      SearchState.visibilityObserver = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
          var _a;
          const input2 = entry.target;
          if (entry.isIntersecting && entry.intersectionRatio > 0) {
            if (this.isElementVisible(input2)) {
              if (this.isShopifySearchInput(input2) && !SearchState.replacedInputs.has(input2) && !this.isFilterSectionInput(input2)) {
                Log.debug("Search input became visible, initializing search", {
                  name: input2.name,
                  type: input2.type
                });
                this.replaceInput(input2);
                (_a = SearchState.visibilityObserver) == null ? void 0 : _a.unobserve(input2);
                SearchState.observedInputs.delete(input2);
              }
            }
          }
        });
  ...
  init(config) {
    ...
    const checkVisibleInputs = () => {
      ...
      visibilityCheckTimeout = window.setTimeout(() => {
        ...
        inputs.forEach((input) => {
          var _a;
          if (!SearchState.replacedInputs.has(input) && !this.isFilterSectionInput(input) && this.isShopifySearchInput(input)) {
            if (this.isElementVisible(input)) {
              Log.debug("Input became visible on scroll/resize, initializing", { name: input.name });
              this.replaceInput(input);
              if (SearchState.observedInputs.has(input)) {
                (_a = SearchState.visibilityObserver) == null ? void 0 : _a.unobserve(input);
