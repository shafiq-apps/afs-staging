...
function getSharedUtilities() {
  return {
    ...
    QuickAdd: appWindow.QuickAdd || { add: async () => {
    }, addFromForm: async () => {
    }, addVariant: async () => {
    }, showSuccess: () => {
    } },
...
async function createProductModal(handle, modalId) {
  ...
  const { State: State2, Log: Log2, QuickAdd: QuickAdd2, $: $2, Icons: Icons2, Lang: Lang2, SpecialValue: SpecialValue2 } = utils;
  const dialog = $2.el("dialog", "afs-product-modal", { "id": modalId });
  dialog.innerHTML = `
      <div class="afs-product-modal__container">
        <div class="afs-product-modal__close-container">
          <button class="afs-product-modal__close" type="button">${Icons2.close}</button>
        </div>
        <div class="afs-product-modal__content">
          <div class="afs-product-modal__loading" style="padding: 2rem; text-align: center;">
            ${Lang2.labels.loadingProduct}
          </div>
        </div>
      </div>
    `;
  ...
  try {
    ...
    const buildVariantSelector = () => {
      ...
      if (productData.variants.length === 1) {
        ...
        if (variantTitle && $2.equals(variantTitle, SpecialValue2.DEFAULT_TITLE)) {
          return "";
        }
      }
    ...
    const buildImagesHTML = () => {
      ...
      productData.images.forEach((image, index) => {
        ...
        const thumbnailUrl = $2.optimizeImageUrl(image, {
          width: 100,
          height: 100,
          crop: "center",
          format: "webp",
        ...
        const thumbnailSrcset = thumbnailSizes.map((size) => {
          const optimized = $2.optimizeImageUrl(image, {
            width: size,
            height: size,
            crop: "center",
            format: "webp",
      ...
      productData.images.forEach((image, index) => {
        const mainImageUrl = $2.optimizeImageUrl(image, {
          width: 800,
          height: 800,
    ...
    const formatPrice = (price) => {
      return $2.formatMoney(price, State2.moneyFormat || "{{amount}}", State2.currency || "");
    };
    ...
    const comparePriceHTML = currentVariant.compare_at_price && currentVariant.compare_at_price > currentVariant.price ? `<span class="afs-product-modal__compare-price">${formatPrice(currentVariant.compare_at_price)}</span>` : "";
    dialog.innerHTML = `
        <div class="afs-product-modal__container">
          <div class="afs-product-modal__close-container">
            <button class="afs-product-modal__close" type="button">${Icons2.close}</button>
                ...
                <div class="afs-product-modal__buttons">
                  <div class="afs-product-modal__add-to-cart">
                    <div class="afs-product-modal__incrementor">
                      <button class="afs-product-modal__decrease" type="button">${Icons2.minus}</button>
                      <span class="afs-product-modal__count" id="${modalId}-count">1</span>
                      <button class="afs-product-modal__increase" type="button">${Icons2.plus}</button>
                    </div>
                    <button
    ...
    const utils2 = getSharedUtilities();
    const { Log: Log3, Icons: Icons3, Lang: Lang3 } = utils2;
    Log3.error("Failed to load product for modal", { error: error instanceof Error ? error.message : String(error), handle });
    dialog.innerHTML = `
        <div class="afs-product-modal__container">
          <div class="afs-product-modal__close-container">
            <button class="afs-product-modal__close" type="button">${Icons3.close}</button>
          </div>
          <div class="afs-product-modal__content">
            <div style="padding: 2rem; text-align: center;">
              <p>${Lang3.messages.failedToLoadProductModal}</p>
            </div>
          </div>
        </div>
      `;
    setupCloseHandler(dialog, utils2);
  }
  return dialog;
}
function setupModalHandlers(dialog, modalId, product, formatPrice, utils) {
  const { Log: Log2, QuickAdd: QuickAdd2, Lang: Lang2 } = utils;
  ...
  variantButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (product.options) {
        product.options.forEach((option, optionIndex2) => {
          const selectedBtn = dialog.querySelector(
            `.afs-product-modal__option-value[data-option-index="${optionIndex2}"].afs-product-modal__option-value--selected`
          );
          if (selectedBtn) {
            selectedValues[optionIndex2] = selectedBtn.dataset.optionValue || "";
          }
        });
      }
      ...
      const matchingVariant = (_a = product.variants) == null ? void 0 : _a.find((v) => {
  ...
  if (addButton && countDisplay) {
    addButton.addEventListener("click", async () => {
      ...
      try {
        await QuickAdd2.addVariant(parseInt(variantId || "0", 10), quantity);
        closeModal();
      } catch (error) {
        Log2.error("Failed to add to cart from modal", { error: error instanceof Error ? error.message : String(error) });
        alert(Lang2.messages.failedToAddToCart);
      }
    });
  }
  ...
  if (buyButton && countDisplay) {
    buyButton.addEventListener("click", async () => {
      ...
      try {
        ...
        Log2.error("Failed to buy now", { error: error instanceof Error ? error.message : String(error) });
        alert(Lang2.messages.failedToProceedToCheckout);
      }
    });
  }
}
function updateVariantInModal(dialog, modalId, variant, formatPrice, utils) {
  const { Lang: Lang2 } = utils;
  ...
  if (priceContainer) {
    ...
    const comparePriceHTML = variant.compare_at_price && variant.compare_at_price > variant.price ? `<span class="afs-product-modal__compare-price">${formatPrice(variant.compare_at_price)}</span>` : "";
  ...
  if (addButton) {
    ...
    addButton.innerHTML = Lang2.buttons.addToCart;
  }
  const buyButton = dialog.querySelector(`#${modalId}-buy-button`);
  if (buyButton) {
...
function setupCloseHandler(dialog, utils) {
  const closeBtn = dialog.querySelector(".afs-product-modal__close");
  const closeModal = () => {
    if (dialog._slider && typeof dialog._slider.destroy === "function") {
      dialog._slider.destroy();
...
function createQuickViewButton(product) {
  const utils = getSharedUtilities();
  const { $: $2, Icons: Icons2, Lang: Lang2 } = utils;
  if (!product.handle) return null;
  const quickViewBtn = $2.el("button", "afs-product-card__quick-view", {
    "data-product-handle": product.handle,
    "data-product-id": String(product.id || product.productId || product.gid || ""),
    "aria-label": Lang2.buttons.quickView,
    "type": "button"
  });
  const quickViewIcon = $2.el("span", "afs-product-card__quick-view-icon");
  quickViewIcon.innerHTML = Icons2.eye;
  quickViewBtn.appendChild(quickViewIcon);
  return quickViewBtn;
}
function handleQuickViewClick(handle) {
  const utils = getSharedUtilities();
  const { Log: Log2, Lang: Lang2 } = utils;
  if (!handle) return;
  ...
  openModal().catch((error) => {
    Log2.error("Failed to open product modal", { error: error instanceof Error ? error.message : String(error), handle });
    alert(Lang2.messages.failedToLoadProductModal);
  });
}
...
// src/advanced-filter-search.ts
var SpecialValue = /* @__PURE__ */ ((SpecialValue2) => {
  SpecialValue2["ALL"] = "__all__";
  SpecialValue2["CLEAR"] = "__clear__";
  SpecialValue2["DEFAULT_TITLE"] = "Default Title";
...
var EXCLUDED_QUERY_PARAMS = /* @__PURE__ */ new Set(["shop" /* SHOP */, "shop_domain" /* SHOP_DOMAIN */, "keep" /* KEEP */, "cpid" /* CPID */]);
var Lang = {
  buttons: {
    quickAdd: "Quick Add",
...
var State = {
  ...
  keep: null,
  // null, array of strings, or SpecialValue.ALL
...
var UrlManager = {
  parse() {
    ...
    url.searchParams.forEach((value, key) => {
      ...
      else if ($.equals(key, "limit" /* LIMIT */)) params.limit = parseInt(value, 10) || C.PAGE_SIZE;
  ...
  update(filters, pagination, sort) {
    ...
    if (State.keep !== null && State.keep !== void 0) {
      if ($.equals(State.keep, "__all__" /* ALL */)) {
        url.searchParams.set("keep", "__all__" /* ALL */);
...
var API = {
  ...
  shouldSendCpid(filters) {
    var _a;
    if (!((_a = State.selectedCollection) == null ? void 0 : _a.id)) {
      return false;
    }
  ...
  async products(filters, pagination, sort) {
    ...
    if (this.shouldSendCpid(filters)) {
      ...
      Log.debug("cpid not sent to products API", {
        hasCpid: !!((_a = State.selectedCollection) == null ? void 0 : _a.id),
        filters,
    ...
    Object.keys(filters).forEach((k) => {
      ...
      if ($.isPriceRangeKey(k) && v && typeof v === "object" && !Array.isArray(v)) {
        ...
        const max = typeof priceRange.max === "number" && !isNaN(priceRange.max) ? priceRange.max : void 0;
        if (State.priceRangeHandle) {
          params.set(State.priceRangeHandle, `${min !== void 0 ? min : ""}-${max !== void 0 ? max : ""}`);
        } else {
    ...
    const promise = this.fetch(url).then((res) => {
      ...
      Log.info("Products response", {
        ...
        total: ((_b = data.pagination) == null ? void 0 : _b.total) || 0,
  ...
  async filters(filters) {
    ...
    if (State.keep !== null && State.keep !== void 0) {
      if ($.equals(State.keep, "__all__" /* ALL */)) {
        ...
        params.set("keep", "__all__" /* ALL */);
        Log.debug(`Keep filters: ${"__all__" /* ALL */} - excluded all filters from aggregation query`);
      } else {
        ...
        } else if (State.keep !== null && !$.equals(State.keep, "__all__" /* ALL */) && typeof State.keep === "string") {
...
var DOM = {
  ...
  renderFilters(filters) {
    ...
    this.filtersContainer.querySelectorAll(".afs-filter-group").forEach((g) => {
      ...
      if (key) {
        ...
        states.set(key, {
          ...
          search: (searchInput == null ? void 0 : searchInput.value) || ""
        });
      }
    });
    ...
    validFilters.forEach((filter) => {
      ...
      const collapsed = (saved == null ? void 0 : saved.collapsed) !== void 0 ? saved.collapsed : filter.collapsed === true || filter.collapsed === "true" || filter.collapsed === 1;
      ...
      Log.debug("Filter group created", {
        ...
        valuesCount: ((_a = filter.values) == null ? void 0 : _a.length) || 0
      });
  ...
  createPriceRangeGroup(filter, savedStates = null) {
    ...
    const collapsed = (_a = saved == null ? void 0 : saved.collapsed) != null ? _a : filter.collapsed === true;
    group.setAttribute("data-afs-collapsed", collapsed ? "true" : "false");
    const header = $.el("div", "afs-filter-group__header");
    ...
    header.appendChild(toggle);
    const isPriceRangeActive = State.filters.priceRange && (State.filters.priceRange.min !== minRange || State.filters.priceRange.max !== maxRange);
    ...
    const formatPrice = (val) => `$${parseFloat(String(val)).toFixed(0)}`;
  ...
  createProduct(p) {
    ...
    if (p.imageUrl || p.featuredImage) {
      ...
      const baseImageUrl = ((_a = p.featuredImage) == null ? void 0 : _a.url) || ((_b = p.featuredImage) == null ? void 0 : _b.urlFallback) || p.imageUrl || "";
      if (baseImageUrl) {
        ...
        img.onerror = function() {
          var _a2;
          const fallbackUrl = ((_a2 = p.featuredImage) == null ? void 0 : _a2.urlFallback) || baseImageUrl;
  ...
  renderApplied(filters) {
    ...
    Object.keys(filters).forEach((key) => {
      const value = filters[key];
      if ($.equals(key, "search" /* SEARCH */) && value && typeof value === "string" && value.trim()) {
        ...
        if (hasMin && hasMax) {
          activeFilters.push({ handle: key, label: `${Lang.labels.price}$${priceRange.min} - $${priceRange.max}`, value: "__clear__" /* CLEAR */ });
        } else if (hasMin) {
          activeFilters.push({ handle: key, label: `${Lang.labels.price}$${priceRange.min}+`, value: "__clear__" /* CLEAR */ });
        } else if (hasMax) {
          activeFilters.push({ handle: key, label: `${Lang.labels.price}Up to $${priceRange.max}`, value: "__clear__" /* CLEAR */ });
        }
    ...
    if ((_a = State.selectedCollection) == null ? void 0 : _a.id) {
      const hasCollectionFilter = Object.keys(filters).some((key) => {
        const metadata = State.filterMetadata.get(key);
        return ($.isCollectionOptionType(metadata == null ? void 0 : metadata.optionType) || $.isCollectionKey(key)) && Array.isArray(filters[key]) && filters[key].includes(String(State.selectedCollection.id));
      });
      if (!hasCollectionFilter) {
        const collection = (_b = State.collections) == null ? void 0 : _b.find((c) => {
          const cId = String(c.id || c.gid || c.collectionId || "");
          return cId && String(cId) === String(State.selectedCollection.id);
        });
        const collectionName = (collection == null ? void 0 : collection.title) || (collection == null ? void 0 : collection.label) || (collection == null ? void 0 : collection.name) || "Collection";
        activeFilters.push({
          handle: "cpid" /* CPID */,
  ...
  showLoading() {
    ...
    if (this.productsGrid) {
      ...
      const pageSize = ((_a = State.pagination) == null ? void 0 : _a.limit) || C.PAGE_SIZE || 24;
  ...
  reloadPage(filters, pagination, sort) {
    ...
    Object.keys(filters || {}).forEach((key) => {
      ...
      if (Array.isArray(value) && value.length > 0) {
        ...
        const max = typeof priceRange.max === "number" && !isNaN(priceRange.max) ? priceRange.max : void 0;
        if (State.priceRangeHandle) {
          url.searchParams.set(State.priceRangeHandle, `${min !== void 0 ? min : ""}-${max !== void 0 ? max : ""}`);
        } else {
          if (min !== void 0) url.searchParams.set("priceMin", String(min));
          if (max !== void 0) url.searchParams.set("priceMax", String(max));
        }
...
var clearCpidIfFiltersPresent = (filters) => {
  var _a;
  if (!((_a = State.selectedCollection) == null ? void 0 : _a.id)) {
    return;
  }
  const hasFilters = Object.keys(filters || {}).some((key) => {
    ...
    if ($.equalsAny(key, "page" /* PAGE */, "limit" /* LIMIT */, "sort" /* SORT */, "size" /* SIZE */)) return false;
    ...
    if ($.equals(key, "search" /* SEARCH */) && (!value || typeof value !== "string" || !value.trim())) return false;
    return true;
  });
  if (hasFilters) {
...
var Filters = {
  ...
  toggle(handle, value) {
    ...
    const isCollectionFilter = (metadata == null ? void 0 : metadata.optionType) === "Collection" || handle === "collections";
    if (isCollectionFilter && ((_a = State.selectedCollection) == null ? void 0 : _a.id)) {
    ...
    DOM.showLoading();
    if (window.innerWidth <= 768 && ((_b = DOM.filtersContainer) == null ? void 0 : _b.classList.contains("afs-filters-container--open"))) {
  ...
  updatePriceRange(min, max) {
    ...
    const priceFilter = State.availableFilters.find((f) => $.isPriceRangeOptionType(f.optionType) || $.equals(f.optionKey, "priceRange" /* PRICE_RANGE */));
  ...
  applyProductsOnly: $.debounce(async () => {
    ...
    try {
      ...
      Log.info("Products fetched successfully", { count: ((_a = data.products) == null ? void 0 : _a.length) || 0 });
      ...
      if (State.fallbackProducts && State.fallbackProducts.length > 0) {
        ...
        if (DOM.sortSelect) {
          if ($.isBestSelling(State.sort.field)) {
            ...
            const direction = $.equals(State.sort.order, "asc" /* ASC */) ? "ascending" /* ASCENDING */ : "descending" /* DESCENDING */;
    ...
    try {
      ...
      try {
        ...
        if (Array.isArray(filtersData.filters)) {
          ...
          State.priceRangeHandle = (priceFilter == null ? void 0 : priceFilter.handle) || State.priceRangeHandle;
          if ((_a = State.selectedCollection) == null ? void 0 : _a.id) {
            const collectionFilter = State.availableFilters.find(
              (f) => f.optionType === "Collection" || $.isCollectionKey(f.queryKey) || $.isCollectionKey(f.handle)
            );
            if (collectionFilter) {
  ...
  attach() {
    ...
    DOM.container.addEventListener("click", (e) => {
      ...
      if (action === "clear-all") {
        ...
        if ((_b = State.selectedCollection) == null ? void 0 : _b.id) {
          State.selectedCollection.id = null;
        ...
        if ($.equals(key, "cpid" /* CPID */)) {
          if ((_c = State.selectedCollection) == null ? void 0 : _c.id) {
            State.selectedCollection.id = null;
        ...
        if (State.filters[handle]) {
          if (isCollectionFilter && ((_f = State.selectedCollection) == null ? void 0 : _f.id)) {
            const filterValues = State.filters[handle];
            const originalCpid = State.selectedCollection.id;
            const cpidInValues = Array.isArray(filterValues) && filterValues.some((v) => String(v) === String(originalCpid));
            if (cpidInValues) {
              State.selectedCollection.id = null;
    ...
    window.addEventListener("popstate", () => {
      ...
      if (params.keep !== void 0) {
        if (params.keep === "__all__" /* ALL */ || typeof params.keep === "string" && $.equals(params.keep, "__all__" /* ALL */)) {
          State.keep = "__all__" /* ALL */;
      ...
      if (State.priceRangeHandle && Array.isArray(priceRangeFilterValue) && priceRangeFilterValue.length > 0) {
        ...
        if (parts.length === 2) {
          const min = parts[0].trim() ? parseFloat(parts[0]) : void 0;
          const max = parts[1].trim() ? parseFloat(parts[1]) : void 0;
...
var AFS = {
  init(config = {}) {
    var _a, _b, _c, _d;
    try {
      Log.init(config.enableLogging !== void 0 ? config.enableLogging : config.debug);
      ...
      if (config.keep !== void 0) {
        if ($.equalsAny(config.keep, "__all__" /* ALL */, "__ALL__")) {
          State.keep = "__all__" /* ALL */;
  ...
  async load() {
    ...
    try {
      ...
      try {
        ...
        Log.info("Filters loaded", { filtersCount: ((_a = filtersData.filters) == null ? void 0 : _a.length) || 0 });
      } catch (filterError) {
      ...
      State.filterMetadata = Metadata.buildFilterMetadata(State.availableFilters);
      const priceFilter = State.availableFilters.find((f) => $.isPriceRangeOptionType(f.optionType));
      State.priceRangeHandle = (priceFilter == null ? void 0 : priceFilter.handle) || State.priceRangeHandle || null;
      const priceRangeFilterValue = State.priceRangeHandle ? State.filters[State.priceRangeHandle] : null;
      if (State.priceRangeHandle && Array.isArray(priceRangeFilterValue) && priceRangeFilterValue.length > 0) {
        const raw = String(priceRangeFilterValue[0] || "");
        const parts = raw.split("-");
      ...
      if (urlParams.keep !== void 0) {
        if (urlParams.keep === "__all__" /* ALL */ || typeof urlParams.keep === "string" && $.equals(urlParams.keep, "__all__" /* ALL */)) {
          State.keep = "__all__" /* ALL */;
      ...
      Log.info("Products loaded", { count: ((_c = productsData.products) == null ? void 0 : _c.length) || 0, total: ((_d = productsData.pagination) == null ? void 0 : _d.total) || 0 });
      ...
      if (!hasProducts && State.fallbackProducts && State.fallbackProducts.length > 0) {
        Log.warn("API returned no products, using fallback products from Liquid", {
          apiProductsCount: ((_e = productsData.products) == null ? void 0 : _e.length) || 0,
          fallbackCount: State.fallbackProducts.length
        });
        State.usingFallback = true;
        State.products = State.fallbackProducts;
      ...
      DOM.hideLoading();
      Log.error("Load failed", { error: e instanceof Error ? e.message : String(e), stack: e instanceof Error ? e.stack : void 0, shop: State.shop, apiBaseURL: API.baseURL });
      if (State.fallbackProducts && State.fallbackProducts.length > 0) {
...
var appWindow2 = window;
appWindow2.DOM = DOM;
appWindow2.AFS_State = State;
appWindow2.AFS_API = API;
appWindow2.AFS_LOG = Log;
appWindow2.QuickAdd = QuickAdd;
appWindow2.$ = $;
appWindow2.Icons = Icons;
appWindow2.Lang = Lang;
appWindow2.SpecialValue = SpecialValue;
if (typeof window !== "undefined") {
  appWindow2.AFS = AFS;
